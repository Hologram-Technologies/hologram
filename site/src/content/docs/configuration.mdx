...
Enable source frontend features on `hologram-compiler` when a build needs to
...
File-backed `const_ref` tensors are resolved and hash-checked at compile time.
...
Runtime sessions never reopen `const_ref` source paths; the validated bytes are
...
SDK bindings should read `hologram_last_error_code()` and
