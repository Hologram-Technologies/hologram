...
```ts
import { loadWasmBinding } from "@hologram/wasm";
import { Graph, Session, compileSource, f32 } from "@hologram/sdk";
...
worker assumptions belong in `@hologram/native`, not in this package.
...
into the `@hologram/sdk` error classes. Browser drivers should use the same
