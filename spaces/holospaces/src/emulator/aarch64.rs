...
impl Cpu {
    ...
    /// [`KappaStore`](hologram_space::KappaStore) by **streaming**
    ...
    pub fn boot_linux_disk_streamed<R: FnMut(u64, &mut [u8])>(
        ...
        kernel: &[u8],
        bootargs: &str,
        store: alloc::boxed::Box<dyn hologram_space::KappaStore>,
        sector_count: u64,
        read: R,
    ) -> Self {
    ...
    fn boot_linux_inner(
        ...
        // from the moment of boot, and the guest mounts it (tag `hsworkspace`)
        // through the devicetree slot advertised below.
        let virtio9p = virtio9p.or_else(|| {
            Some(super::Virtio9p::new(
                super::ninep::Fs9p::new(),
                super::WORKSPACE_TAG,
            ))
        });
