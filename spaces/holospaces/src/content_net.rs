...
//! supplies the mechanism: [`hologram_net::bare::BareNetSync`] is a
//! [`KappaSync`](hologram_space::KappaSync) that speaks the uor-native
//! frame protocol (`fetch`/`announce`/`discover`, **verify-on-receipt** —
//! SPINE-4) over a [`NetworkInterface`](hologram_space::NetworkInterface) HAL.
...
use hologram_net::bare::{BareNetSync, LocalIterator, LocalResolver};
use hologram_space::{Bytes, KappaLabel71, KappaStore, KappaSync};
