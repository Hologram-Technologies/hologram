...
//! ([`hologram_space::verify_kappa_axis`]). holospaces walks the OCI
...
use hologram_space::{verify_kappa_axis, KappaStore, StoreError};
...
pub fn sha256_digest(bytes: &[u8]) -> String {
    String::from_utf8(
        hologram_space::address_bytes_axis("sha256", bytes)
...
mod tests {
    ...
    fn sha256_label(bytes: &[u8]) -> String {
        let label = hologram_space::address_bytes_axis("sha256", bytes).unwrap();
