...
//! ([`Kappa`] = [`hologram_space::KappaLabel71`], `<axis>:<hex>` =
...
//! [`verify`] wrap `hologram_space`'s σ-axis functions. holospaces
...
/// (`hologram_space::REGISTRY` — `ContainerManifest`, `CapabilitySet`,
...
pub fn registry() -> Vec<(RealizationId, RefExtractor)> {
    ...
    reg.extend_from_slice(hologram_space::REGISTRY);
    reg
}
...
/// via [`hologram_space::address_bytes`] (Law L2).
#[must_use]
...
/// via [`hologram_space::verify_kappa`].
///
...
impl Realization for Holospace {
    const IRI: hologram_space::RealizationId = Holospace::IRI;
