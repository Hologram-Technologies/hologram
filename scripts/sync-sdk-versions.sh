...
# The two leaf packages (native N-API, wasm driver) declare a `peerDependencies` on the `@tryhologram/sdk`
...
# Sync / gate the `@tryhologram/sdk` peer-dependency pin in the leaf packages.
for f in "${PEER_FILES[@]}"; do
  [ -f "$f" ] || continue
  cur="$(grep -m1 '"@tryhologram/sdk"' "$f" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)"
