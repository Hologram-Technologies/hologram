...
fn main() {
    ...
    println!("dedup     : 2 identical artifacts → 1 κ ({} object in cache)", cache.approximate_count());
    ...
    let build = cache
        .put("blake3", &ContainerManifest { code: obj_a, initial_state: src, parameters: toolchain }.canonicalize())
        .unwrap();
    ...
    println!("gc        : reclaimed {} stale artifact(s); inputs retained = {}", evicted, cache.contains(&src) && cache.contains(&toolchain) && cache.contains(&obj_a));
