...
| **KD** | κ-Distribution protocol (spec 003) — content-addressed `/v2/` registry, conformance Levels 1–5 (blobs · tags · edges · composition/witness/schema · GC/admission/federation); identity stays κ-only, tags/human-names are a resolution overlay | Rust tests (`hologram-net --features kd`, …) run by the **non-blocking** `kd-conformance` job; promoted to blocking at ratification (plan Phase 8) |
...
## KD — κ-Distribution protocol (spec 003; `/v2/` registry; non-BDD, non-blocking until ratified)

> The κ-Distribution `/v2/` registry binding — `specs/prompts/003-KAPPA_DISTRIBUTION_SPEC_v2.md`,
> built level-by-level as hologram's conforming implementation of the external UOR-REGISTRY /
> UOR-RESOLUTION standard (incubating as `uor-distribution`). Identity stays κ-only (Law 2 / SPINE-1):
> the `{path}` prefix and the future tag/human-name overlay never participate in addressing. Rows are
> 🟡 while witnessed by passing tests but gated only by the **non-blocking** `kd-conformance` CI job;
> each flips ✅ when that job becomes a required check (plan Phase 8). **Level 1 (blobs) is
> implemented** (KD-1..6, over `hologram-net --features kd`); the incubating standard crate
> `crates/uor-distribution` carries the cross-registry **byte contracts** (edge/pin/witness/tag-snapshot
> canonical forms), beginning with the edge canonical form (**KD-10**). **All five conformance levels
> are fully implemented** (KD-1..23): L1 blobs + L2 tags (KD-1..9, κ-only per Law 2) over
> `hologram-net --features kd`; and L3 edges (KD-10..13), L4 composition/witnesses/schemas (KD-14..17),
> and L5 GC/finalizers/admission/federation (KD-18..23) in `crates/hologram-registry` — the leaf
> service crate that runs the `/v2/` server, delegating L1/L2 to `hologram-net` and adding L3–L5 over
> the shared `uor-distribution` canonical forms (it may consume the unpublished standard; the published
> core crates cannot). Admission filters are enforced on the write path and tags are GC roots across
> that boundary (KD-22/23, via the `handle_v2_admitted` / `tags_for` seams). These crates are
> structured for **extraction** to their own repos. Not a BDD class (witnessed by Rust tests, like
> AS/KC/CC) so the honesty meta-gate ignores it.

| ID | Statement | Enforcement | Witness | Status |
|---|---|---|---|---|
| **KD-1** | `PUT /v2/{path}/blobs/{κ}` re-hashes the content and rejects a κ that does not match (`409 DIGEST_INVALID`), and is idempotent: `201` on first store, `200` on re-put of identical content (§5.1 verify-on-put + idempotent put). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd1_verify_on_put_idempotent_and_digest_mismatch` | 🟡 |
| **KD-2** | `HEAD`/`GET /v2/{path}/blobs/{κ}` return the exact stored bytes with `X-Kappa-Label`/`X-Kappa-Axis`; the served label re-hashes to κ (verify-on-receipt) and the bytes are byte-identical (§3.1 exact-bytes); an absent κ is `404`. | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd2_head_get_roundtrip_verify_on_receipt_exact_bytes` | 🟡 |
| **KD-3** | `GET /v2/` advertises the protocol version (`{"kappa-distribution":"2.0.0"}`) and `GET /v2/_health/{live,ready,startup}` respond `200` (§6.2 version check + §6.14 health). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd3_version_check_and_health` | 🟡 |
| **KD-4** | Multi-label push `PUT /v2/{path}/blobs/{κ}?also={κ2}` verifies **every** provided κ against the content, stores the blob once and indexes it under each σ-axis (both addresses resolve); any mismatch rejects the whole push and stores nothing (§4.6). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd4_multi_label_push_all_verified_stored_once` | 🟡 |
| **KD-5** | Chunked upload (`POST` session → sequential `PATCH` chunks → `PUT ?kappa=` complete) assembles the whole blob and verifies it against the declared κ; an out-of-order chunk is `416`, `GET` on the session recovers the byte cursor for resume, and `DELETE` cancels (§6.4). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd5_chunked_upload_recovery_and_cancel` | 🟡 |
| **KD-6** | Mount `POST /v2/{path}/blobs/uploads/?mount={κ}` binds an already-present blob without re-upload (`201`); an absent blob falls back to a normal upload session (`202`) (§6.7). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd6_mount_existing_blob_or_fall_back_to_upload` | 🟡 |
| **KD-7** | Tag bind (`PUT /v2/{path}/manifests/{tag}`) stores content then points the tag at its κ (content-before-tag); resolve (`GET`) returns the content with κ echoed; `DELETE` removes the binding but not the content (§6.5, §7.5). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd7_tag_bind_resolve_content_before_tag_and_delete` | 🟡 |
| **KD-8** | `GET /v2/{path}/tags/list` returns tags in ASCIIbetical order with asc/desc, cursor pagination (`last` is a tag name, not an index) and a `Link` only when more pages remain; `n=0` is an empty list with no `Link` (§6.5). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd8_tag_list_order_and_cursor_pagination` | 🟡 |
| **KD-9** | **Law-2 guard:** identity is the κ, not the tag — two tag names over identical content resolve to the same κ, a colon-bearing `{version}` resolves as a direct κ bypassing tags, and `tag_set_if` (`If-Match`) CAS moves the pointer only (stale → `409 TAG_CONFLICT`), never the immutable content (§5.7, Law 2 / SPINE-1). | test (`--features kd`) | `hologram-net/src/http/kd.rs::tests::kd9_identity_is_kappa_not_tag_with_cas` | 🟡 |
| **KD-10** | The **edge canonical byte form** (the cross-registry addressing contract) matches the Appendix C layout byte-for-byte — `source ‖ 0x00 ‖ relation ‖ 0x00 ‖ target ‖ 0x00 ‖ u32-BE(len) ‖ metadata` — with a big-endian metadata length; distinct triples yield distinct bytes (§3.4, Appendix C). | test (`uor-distribution`) | `uor-distribution/src/edge.rs::tests::kd10_edge_canonical_matches_appendix_c_layout` | 🟡 |
| **KD-11** | An edge is a content-addressed blob whose κ is built from the **shared** standard canonical form (KD-10, no re-definition) hashed under the **source's** σ-axis (§3.4); it is deterministic, idempotent, and storable/retrievable by its own κ, and a cross-axis edge follows the source axis. | test (`hologram-registry`) | `hologram-registry/src/edge.rs::tests::kd11_edge_kappa_inherits_source_axis_via_shared_canonical_form` | 🟡 |
| **KD-12** | `PUT /v2/{path}/edges/` verifies the source exists (`409 EDGE_SOURCE_ABSENT`), tolerates an absent target (lazy replication), is idempotent (`201`→`200`), and the created edge is retrievable as a blob via the L1 GET (§5.3, §6.6). | test (`hologram-registry`) | `hologram-registry/tests/edges_http.rs::kd12_edge_put_source_exists_absent_target_tolerated_idempotent` | 🟡 |
| **KD-13** | `GET /v2/{path}/edges/{κ}` queries the edge index by direction (outbound/inbound/both) and relation; `DELETE /v2/{path}/edges/{edge-κ}` removes the edge from the index (§3.4, §6.6). | test (`hologram-registry`) | `hologram-registry/tests/edges_http.rs::kd13_edge_index_query_by_direction_relation_and_delete` | 🟡 |
| **KD-14** | The five **composition canonical forms** match spec Appendix B: g2 is commutative, f4 is involution-invariant, e6 tags by degree, e7 is quarter-permutation-invariant, e8 is identity; σ-axis homogeneity + arity are enforced; the §3.6 witness header round-trips. | test (`uor-distribution`) | `uor-distribution/src/compose.rs::tests::kd14_composition_canonical_forms_and_witness_header` | 🟡 |
| **KD-15** | `POST /v2/{path}/compose/{op}` applies the operation (composed κ = hash of the shared canonical form), returns composed + witness + operation, is commutative for g2, stores a retrievable composed blob, creates composed-of/witness-of edges, and rejects mixed-axis operands with `422 AXIS_MISMATCH` (§5.4, §6.9). | test (`hologram-registry`) | `hologram-registry/tests/compose_http.rs::kd15_compose_ops_provenance_and_axis_mismatch` | 🟡 |
| **KD-16** | `GET /v2/{path}/witnesses/{κ}` resolves the witness for a composed κ via its witness-of edge; the witness carries the §3.6 self-describing header and its trace replays (re-hashes) to the composed κ (§3.6, §6.8). | test (`hologram-registry`) | `hologram-registry/tests/compose_http.rs::kd16_witness_retrieval_and_replay` | 🟡 |
| **KD-17** | A schema registers/retrieves as a content-addressed blob (`PUT`/`GET /v2/{path}/schemas/{scope}`) and is **admission-only**: the same content addresses to the same κ with or without a schema for its scope (§3.7, §6.10). | test (`hologram-registry`) | `hologram-registry/tests/compose_http.rs::kd17_schema_register_retrieve_and_admission_only` | 🟡 |
| **KD-18** | GC `pin` + `sweep` + `status`: a pinned blob and what it `owns` are reachable and retained; an unpinned, unreachable blob is reported evictable; the sweep marks reachability from roots over owns/composed-of edges (§6.11, §9). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd18_gc_pin_sweep_reachability_status` | 🟡 |
| **KD-19** | A finalizer (a pin carrying a controller) blocks `unpin` with `409 FINALIZER_OUTSTANDING` until it is explicitly released (§3.8, §6.11). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd19_finalizer_blocks_unpin_until_released` | 🟡 |
| **KD-20** | Admission filters register/list/remove as content-addressed blobs (`/v2/{path}/filters`), and the enforcement primitive is all-accept + fail-closed (a `deny:` filter rejects matching content) (§6.12, §10). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd20_admission_filters_register_list_remove_and_fail_closed` | 🟡 |
| **KD-21** | Federation fetch is verify-on-receipt: a blob absent from registry A is fetched from peer registry B, re-hashed against the requested κ before it is accepted/cached, and an absent κ yields no content (§8.3, §11.6). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd21_federation_fetch_verifies_on_receipt` | 🟡 |
| **KD-22** | Admission filters are **enforced on the blob-write path**: a `PUT` whose content a registered filter rejects fails closed with `422 FILTER_REJECTED` and stores nothing; clean content is admitted (the registry runs `filter::admit` over the delegated L1 write via the `handle_v2_admitted` seam) (§5.1, §10). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd22_admission_filter_enforced_on_blob_write` | 🟡 |
| **KD-23** | A **tagged blob is a GC root**: with no pin, a tag keeps its blob reachable through a sweep while an unreferenced orphan chain is evictable — GC reaches tags across the L1/L2 delegation boundary via `kd::tags_for` (§9.1). | test (`hologram-registry`) | `hologram-registry/tests/l5_http.rs::kd23_tagged_blob_is_a_gc_root` | 🟡 |
