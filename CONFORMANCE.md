...
| **RM-25** | Run a holospace from Rust: `Manager::sign_in` → `provision(Source)` → `open` → `boot` → `suspend` (κ snapshot) → `terminate`, over the real Wasmtime engine. | BDD scenario | `s7_readme/holospace-manager.feature::provision open boot and suspend a holospace to a κ snapshot` | ✅ |
...
| **CC-45** | dogfood — holospaces builds in its own real devcontainer and survives 4096 dynamic fork/exec cycles before compiling with its real GCC | test (required release V&V; no ignore/filter) | `spaces/holospaces/tests/cc45_dogfood.rs::holospaces_builds_in_its_own_real_devcontainer` | ✅ |
...
| **CC-47** | lifecycle architecture parity — AArch64 and x86-64 suspend to κ-addressed CPU + RAM + sparse κ-disk snapshots, restore byte-identically after drop, and continue deterministically | test (release V&V; no ignore/filter) | `spaces/holospaces/tests/cc47_lifecycle_parity.rs::aarch64_suspend_drop_restore_preserves_cpu_ram_disk_and_continuation` | ✅ |
