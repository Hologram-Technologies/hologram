...
#[no_mangle]
pub extern "C" fn hologram_inputs_new() -> *mut GraphInputs {
    into_handle(GraphInputs::new())
}

/// Set input data at `index`. Copies `data_len` bytes from `data_ptr`.
#[no_mangle]
pub extern "C" fn hologram_inputs_set(
...
#[no_mangle]
pub extern "C" fn hologram_inputs_free(inputs: *mut GraphInputs) {
...
/// Number of outputs.
#[no_mangle]
pub extern "C" fn hologram_outputs_len(outputs: *const GraphOutputs) -> i32 {
...
#[no_mangle]
pub extern "C" fn hologram_outputs_get(
...
#[no_mangle]
pub extern "C" fn hologram_outputs_name(
    outputs: *const GraphOutputs,
    index: usize,
) -> *const c_char {
    let Some(o) = borrow_handle(outputs) else {
...
#[no_mangle]
pub extern "C" fn hologram_outputs_by_name(
    outputs: *const GraphOutputs,
    name: *const c_char,
    out_ptr: *mut *const u8,
    out_len: *mut usize,
...
#[no_mangle]
pub extern "C" fn hologram_outputs_free(outputs: *mut GraphOutputs) {
...
mod tests {
    ...
    fn compile_test_graph() -> *mut hologram_compiler::CompilationOutput {
        ...
        hologram_graph_builder_node_with_inputs(b, 3, 0, inputs.as_ptr(), 1); // Lut(Sigmoid)
                                                                              // ADR-053: Sigmoid (idx 1) gets [1] for v3 shape coverage.
        let shape = [1usize];
    ...
    fn outputs_by_name() {
        ...
        let mut ptr: *const u8 = std::ptr::null();
        let mut len: usize = 0;
        let rc = hologram_outputs_by_name(outputs, name.as_ptr(), &mut ptr, &mut len);
        assert_eq!(rc, 0);
        assert_eq!(len, 1);
    ...
    fn free_null_handles_safe() {
        hologram_inputs_free(std::ptr::null_mut());
        hologram_outputs_free(std::ptr::null_mut());
    }

    #[test]
    fn inputs_set_null_data_with_zero_len() {
        let inp = hologram_inputs_new();
        let rc = hologram_inputs_set(inp, 0, std::ptr::null(), 0);
        assert_eq!(rc, 0);
        hologram_inputs_free(inp);
    }
    ...
    fn inputs_set_null_data_with_nonzero_len() {
        let inp = hologram_inputs_new();
        let rc = hologram_inputs_set(inp, 0, std::ptr::null(), 10);
        assert!(rc < 0);
        hologram_inputs_free(inp);
    }
}
