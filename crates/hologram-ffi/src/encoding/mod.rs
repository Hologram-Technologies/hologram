...
#[no_mangle]
pub extern "C" fn hologram_encoding_embed(encoding_id: i32, value: f64) -> u8 {
...
#[no_mangle]
pub extern "C" fn hologram_encoding_lift(encoding_id: i32, byte: u8) -> f64 {
    match get_encoding(encoding_id) {
...
#[no_mangle]
pub extern "C" fn hologram_lut_apply(lut_op: i32, byte: u8) -> u8 {
...
#[no_mangle]
pub extern "C" fn hologram_prim_apply_unary(prim_op: i32, byte: u8) -> u8 {
...
#[no_mangle]
pub extern "C" fn hologram_prim_apply_binary(prim_op: i32, a: u8, b: u8) -> u8 {
    use hologram_core::op::PrimOp;
    let all = [
        PrimOp::Neg,
        PrimOp::Bnot,
...
fn get_encoding(id: i32) -> Option<Box<dyn hologram_core::encoding::Encoding>> {
    use hologram_core::encoding::*;
    match id {
...
mod tests {
    ...
    fn unsigned_round_trip() {
        let byte = hologram_encoding_embed(2, 0.5);
        let val = hologram_encoding_lift(2, byte);
        assert!((val - 0.5).abs() < 0.01);
    }

    #[test]
    fn raw_encoding() {
        assert_eq!(hologram_encoding_embed(3, 42.0), 42);
        assert_eq!(hologram_encoding_lift(3, 42) as u8, 42);
    }

    #[test]
    fn invalid_encoding() {
        let byte = hologram_encoding_embed(99, 1.0);
        assert_eq!(byte, 0);
        let val = hologram_encoding_lift(99, 0);
        assert!(val.is_nan());
    }
    ...
    fn lut_sigmoid() {
        ...
        let a = hologram_lut_apply(0, 128);
        let b = hologram_lut_apply(0, 128);
        assert_eq!(a, b);
    }

    #[test]
    fn lut_invalid() {
        let result = hologram_lut_apply(99, 0);
        assert_eq!(result, 0);
    }

    #[test]
    fn prim_unary_neg() {
        let result = hologram_prim_apply_unary(0, 1);
        assert_eq!(result, 255); // wrapping neg
    }

    #[test]
    fn prim_binary_add() {
        let result = hologram_prim_apply_binary(4, 10, 20);
        assert_eq!(result, 30);
    }

    #[test]
    fn prim_binary_on_unary_errors() {
        let result = hologram_prim_apply_binary(0, 1, 2);
        assert_eq!(result, 0); // Neg is unary, should error
    }

    #[test]
    fn prim_invalid_index() {
        assert_eq!(hologram_prim_apply_unary(99, 0), 0);
        assert_eq!(hologram_prim_apply_binary(99, 0, 0), 0);
    }
}
