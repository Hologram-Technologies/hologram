...
impl ShapeDescriptor {
    pub fn rank1(d0: u64) -> Self {
        let mut dims = [0u64; 8];
        dims[0] = d0;
        Self { rank: 1, dims, dims_overflow: None }
    }
    ...
    pub fn dim(&self, i: usize) -> Option<u64> {
        if i < self.rank as usize {
            if i < 8 {
                ...
                self.dims_overflow.as_ref().and_then(|d| d.get(i - 8).copied())
            }
        } else {
            None
        }
    ...
    pub fn total_elements(&self) -> u64 {
        ...
        for i in 0..r.min(8) { p = p.saturating_mul(self.dims[i]); }
        if let Some(overflow) = &self.dims_overflow {
            for &d in overflow.iter() { p = p.saturating_mul(d); }
        }
        p
    }
}
