...
use hologram_space::{
    ...
    KappaSync, Realization, RuntimeError,
};
use hologram_space::{
    CapabilitySet, ChainCompaction, ContainerManifest, Delegation, ErrorEvent, Snapshot,
};
// The container-engine seam now lives in the space contract (`hologram_space::engine`).
// Re-export it here so the runtime's own API surface — and its orchestration below —
// keep referring to `ContainerEngine` / `HostContext` / `ContainerIntents` unqualified.
pub use hologram_space::{ContainerEngine, ContainerIntents, HostContext};
use spin::Mutex;
