...
fn load_graph(path: &PathBuf) -> Result<holo_graph::Graph, CliError> {
...
fn run_compiler(
    graph: holo_graph::Graph,
    enable_fusion: bool,
) -> Result<holo_compiler::CompilationOutput, CliError> {
    holo_compiler::CompilerBuilder::new(graph)
        .fuse(enable_fusion)
        .build()
        .map_err(CliError::from)
}
