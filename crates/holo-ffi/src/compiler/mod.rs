...
use holo_compiler::{CompilationOutput, CompilerBuilder};
...
#[no_mangle]
pub extern "C" fn holo_compile(graph: *mut Graph) -> *mut CompilationOutput {
    if graph.is_null() {
...
#[no_mangle]
pub extern "C" fn holo_compile_no_fuse(graph: *mut Graph) -> *mut CompilationOutput {
    if graph.is_null() {
        set_last_error("null graph handle");
        return std::ptr::null_mut();
    }
...
#[no_mangle]
pub extern "C" fn holo_compilation_archive_ptr(output: *const CompilationOutput) -> *const u8 {
...
#[no_mangle]
pub extern "C" fn holo_compilation_archive_len(output: *const CompilationOutput) -> usize {
    match borrow_handle(output) {
...
#[no_mangle]
pub extern "C" fn holo_compilation_stats_nodes(output: *const CompilationOutput) -> u32 {
    match borrow_handle(output) {
...
#[no_mangle]
pub extern "C" fn holo_compilation_stats_levels(output: *const CompilationOutput) -> u32 {
    match borrow_handle(output) {
...
#[no_mangle]
pub extern "C" fn holo_compilation_stats_workspace_slots(output: *const CompilationOutput) -> u32 {
    match borrow_handle(output) {
...
#[no_mangle]
pub extern "C" fn holo_compilation_free(output: *mut CompilationOutput) {
...
mod tests {
    ...
    fn build_test_graph() -> *mut Graph {
        ...
        holo_graph_builder_node_from_input(b, 0, 0, 0); // Input wired to graph input 0
        let inputs = [0usize];
    ...
    fn compile_simple_graph() {
        ...
        assert!(!out.is_null());
        assert!(holo_compilation_archive_len(out) > 0);
        assert!(!holo_compilation_archive_ptr(out).is_null());
    ...
    fn compile_no_fuse() {
        let g = build_test_graph();
        let out = holo_compile_no_fuse(g);
        assert!(!out.is_null());
        assert!(holo_compilation_archive_len(out) > 0);
        holo_compilation_free(out);
    }
    ...
    fn stats_on_null_return_zero() {
        ...
        assert_eq!(holo_compilation_stats_nodes(null), 0);
        assert_eq!(holo_compilation_stats_levels(null), 0);
        assert_eq!(holo_compilation_stats_workspace_slots(null), 0);
        assert_eq!(holo_compilation_archive_len(null), 0);
        assert!(holo_compilation_archive_ptr(null).is_null());
    }

    #[test]
    fn free_null_is_safe() {
        holo_compilation_free(std::ptr::null_mut());
    }
    ...
    fn workspace_slots_present() {
        ...
        assert!(holo_compilation_stats_workspace_slots(out) > 0);
        holo_compilation_free(out);
    }
}
