...
#[no_mangle]
pub extern "C" fn holo_graph_builder_new() -> *mut FfiGraphBuilder {
    into_handle(FfiGraphBuilder::new_internal())
}

/// Add a named input. Returns input index (>= 0) or negative error.
#[no_mangle]
pub extern "C" fn holo_graph_builder_input(
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_node(
    builder: *mut FfiGraphBuilder,
    op_kind: i32,
    op_param: i32,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_node_from_input(
    builder: *mut FfiGraphBuilder,
    op_kind: i32,
    op_param: i32,
    graph_input_idx: u32,
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_node_with_inputs(
    builder: *mut FfiGraphBuilder,
    op_kind: i32,
    op_param: i32,
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_edge(
    builder: *mut FfiGraphBuilder,
    source: usize,
    target: usize,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_output(
    builder: *mut FfiGraphBuilder,
    name: *const c_char,
    node_index: usize,
) -> i32 {
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_build(builder: *mut FfiGraphBuilder) -> *mut Graph {
    if builder.is_null() {
...
#[no_mangle]
pub extern "C" fn holo_graph_node_count(graph: *const Graph) -> i32 {
...
#[no_mangle]
pub extern "C" fn holo_graph_builder_free(builder: *mut FfiGraphBuilder) {
    unsafe { free_handle(builder) };
}

/// Free a graph handle.
#[no_mangle]
pub extern "C" fn holo_graph_free(graph: *mut Graph) {
...
fn prim_from_param(p: i32) -> Result<holo_core::op::PrimOp, i32> {
...
fn lut_from_param(p: i32) -> Result<holo_core::op::LutOp, i32> {
    holo_core::op::LutOp::ALL
        .get(p as usize)
        .copied()
        .ok_or_else(|| {
...
mod tests {
    ...
    fn build_empty_graph() {
        ...
        assert_eq!(holo_graph_node_count(g), 0);
        holo_graph_free(g);
    }
    ...
    fn build_linear_chain() {
        let b = holo_graph_builder_new();
        let n0 = holo_graph_builder_node(b, 0, 0);
        assert_eq!(n0, 0);
        let inputs = [0usize];
        let n1 = holo_graph_builder_node_with_inputs(b, 3, 0, inputs.as_ptr(), 1);
        assert_eq!(n1, 1);
        let inputs2 = [1usize];
        let n2 = holo_graph_builder_node_with_inputs(b, 1, 0, inputs2.as_ptr(), 1);
        assert_eq!(n2, 2);
        let g = holo_graph_builder_build(b);
    ...
    fn named_input_output() {
        ...
        let idx = holo_graph_builder_input(b, name_x.as_ptr());
        assert_eq!(idx, 0);
        holo_graph_builder_node(b, 0, 0);
        let name_y = CString::new("y").unwrap();
        let rc = holo_graph_builder_output(b, name_y.as_ptr(), 0);
        assert_eq!(rc, 0);
        let g = holo_graph_builder_build(b);
    ...
    fn null_builder_returns_error() {
        let rc = holo_graph_builder_node(std::ptr::null_mut(), 0, 0);
        assert!(rc < 0);
    }
    ...
    fn invalid_op_kind() {
        let b = holo_graph_builder_new();
        let rc = holo_graph_builder_node(b, 99, 0);
        assert!(rc < 0);
        holo_graph_builder_free(b);
    }

    #[test]
    fn invalid_prim_param() {
        let b = holo_graph_builder_new();
        let rc = holo_graph_builder_node(b, 2, 99);
        assert!(rc < 0);
        holo_graph_builder_free(b);
    }
    ...
    fn invalid_lut_param() {
        let b = holo_graph_builder_new();
        let rc = holo_graph_builder_node(b, 3, 99);
        assert!(rc < 0);
        holo_graph_builder_free(b);
    }
    ...
    fn edge_between_nodes() {
        let b = holo_graph_builder_new();
        holo_graph_builder_node(b, 0, 0);
        holo_graph_builder_node(b, 1, 0);
        let rc = holo_graph_builder_edge(b, 0, 1);
        assert_eq!(rc, 0);
        let g = holo_graph_builder_build(b);
        let graph = borrow_handle(g as *const Graph).unwrap();
    ...
    fn output_invalid_index() {
        ...
        let rc = holo_graph_builder_output(b, name.as_ptr(), 99);
        assert!(rc < 0);
        holo_graph_builder_free(b);
    }
    ...
    fn null_string_returns_error() {
        let b = holo_graph_builder_new();
        let rc = holo_graph_builder_input(b, std::ptr::null());
        assert!(rc < 0);
        holo_graph_builder_free(b);
    }
    ...
    fn build_null_builder_returns_null() {
        let g = holo_graph_builder_build(std::ptr::null_mut());
        assert!(g.is_null());
    }

    #[test]
    fn free_null_is_safe() {
        holo_graph_builder_free(std::ptr::null_mut());
        holo_graph_free(std::ptr::null_mut());
    }
}
