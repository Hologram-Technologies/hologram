...
///
/// For `OutputBuffer::Heap`: resizes the Vec and casts via bytemuck.
/// For `OutputBuffer::Arena`: arena pointers are always 16-byte aligned
/// (enforced by slot offset computation), so the cast always succeeds.
#[inline]
