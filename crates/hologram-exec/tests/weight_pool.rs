...
fn small_constants_are_inlined_in_archive() {
    ...
    let outputs = session.execute(&[
        InputBuffer { bytes: &f32_to_le(&[1.0, 2.0, 3.0, 4.0]) },
    ]).unwrap();
    let expected: Vec<f32> = [1.0_f32, 2.0, 3.0, 4.0].iter()
        .zip([0.5_f32; 4].iter())
        .map(|(a, b)| a + b)
        .collect();
...
fn large_weights_become_content_addressed_references() {
    ...
    let mut graph = Graph::new();
    let shape = graph.shape_registry_mut().intern(ShapeDescriptor::rank1(2048));
    ...
    assert!(archive_size < 16 * 1024,
        "archive {} bytes — body appears double-stored", archive_size);
    assert!(archive_size >= body.len(),
        "archive {} bytes — body lost", archive_size);
}
...
fn duplicate_large_weights_share_storage() {
    ...
    let a = graph.add_node(Node {
        op: GraphOp::Op(OpKind::Add),
        inputs: SmallVec::from_iter([
            InputSource::Node(x), InputSource::Constant(c1),
        ]),
        output_dtype: DTypeId(DTYPE_F32),
        output_shape: shape,
    });
    let b = graph.add_node(Node {
        op: GraphOp::Op(OpKind::Add),
        inputs: SmallVec::from_iter([
            InputSource::Node(a), InputSource::Constant(c2),
        ]),
        output_dtype: DTypeId(DTYPE_F32),
        output_shape: shape,
    });
    ...
    let archive_size = compiled.archive.len();
    assert!(archive_size < 12 * 1024,
        "archive {} bytes — duplicate bodies not deduped", archive_size);
}
