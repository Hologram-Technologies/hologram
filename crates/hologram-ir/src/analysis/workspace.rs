...
//! # Architectural role
//!
//! This module is a **structure finder**, not an optimizer. The slot
//! assignments it produces are derived from the graph's existing liveness
//! structure (computed by [`super::liveness`]). It does not change the graph
//! or make optimization decisions — it reads the natural sharing structure
//! that the source's dependency pattern already implies.
//!
//! # Performance
//!
//! O(N²) worst case for the first-fit-decreasing pass, but in practice the
//! number of compatible slots is small (≤10 for transformer-shaped graphs).
//! **Perf: NEUTRAL** — pure compile-time work.

use crate::analysis::liveness::LivenessInterval;
