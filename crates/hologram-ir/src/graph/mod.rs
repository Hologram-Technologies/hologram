...
// `CustomOpId` and the `Custom { id, arity }` GraphOp variant were removed in
// the v0.2.0 conformance-first refactor. Custom ops violated the carrying
// criterion (an op outside the declared shape) AND introduced a runtime
// function-pointer indirection in the dispatch path. The replacement
