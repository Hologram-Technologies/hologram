...
fn assert_closed_under_primitives<const CAP: usize>(arena: &HoloArena<CAP>) {
...
fn assert_nonempty<const CAP: usize>(arena: &HoloArena<CAP>) {
...
fn matmul_emit_uses_only_primitive_ops() {
    ...
    let mut arena: Box<HoloArena<64>> = Box::new(HoloArena::new());
