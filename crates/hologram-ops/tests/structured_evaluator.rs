...
use hologram_ops::{ScalarEvaluatorU64, ReferenceEvaluator, EvalError};
...
fn match_picks_first_matching_arm() {
    ...
    let lit5 = arena.push(Term::Literal { value: 5, level: WittLevel::W8 }).unwrap();
    let lit3 = arena.push(Term::Literal { value: 3, level: WittLevel::W8 }).unwrap();
    let lit5b = arena.push(Term::Literal { value: 5, level: WittLevel::W8 }).unwrap();
    let lit99 = arena.push(Term::Literal { value: 99, level: WittLevel::W8 }).unwrap();
    let _ = (lit3, lit5b, lit99);
    let arms_start = lit3;
    let m = arena.push(Term::Match {
        scrutinee_index: lit5,
        arms: TermList { start: arms_start, len: 3 },
    }).unwrap();
...
fn match_default_when_no_arm_matches() {
    let mut arena: TermArena<8> = TermArena::new();
    let scrut = arena.push(Term::Literal { value: 100, level: WittLevel::W8 }).unwrap();
    let arm0 = arena.push(Term::Literal { value: 1, level: WittLevel::W8 }).unwrap();
    let _arm1 = arena.push(Term::Literal { value: 2, level: WittLevel::W8 }).unwrap();
    let _arm2 = arena.push(Term::Literal { value: 42, level: WittLevel::W8 }).unwrap();
    let m = arena.push(Term::Match {
        scrutinee_index: scrut,
        arms: TermList { start: arm0, len: 3 },
    }).unwrap();
    let v = ScalarEvaluatorU64::evaluate(&arena, m, &[]).unwrap();
...
fn recurse_descends_to_zero() {
    ...
    let mut arena: TermArena<8> = TermArena::new();
    let measure = arena.push(Term::Literal { value: 5, level: WittLevel::W8 }).unwrap();
    let base = arena.push(Term::Literal { value: 0, level: WittLevel::W8 }).unwrap();
    let step = arena.push(Term::Literal { value: 1, level: WittLevel::W8 }).unwrap();
    let r = arena.push(Term::Recurse {
...
fn try_falls_back_on_error() {
    ...
    let handler = arena.push(Term::Literal { value: 7, level: WittLevel::W8 }).unwrap();
    let t = arena.push(Term::Try {
        body_index: body,
...
fn try_succeeds_when_body_works() {
    let mut arena: TermArena<8> = TermArena::new();
    let body = arena.push(Term::Literal { value: 42, level: WittLevel::W8 }).unwrap();
    let handler = arena.push(Term::Literal { value: 7, level: WittLevel::W8 }).unwrap();
    let t = arena.push(Term::Try {
        body_index: body,
        handler_index: handler,
...
fn recurse_invalid_measure_terminates_via_safety_ceiling() {
    ...
    let mut arena: TermArena<8> = TermArena::new();
    let measure = arena.push(Term::Literal { value: u64::MAX, level: WittLevel::new(64) }).unwrap();
    let base = arena.push(Term::Literal { value: 0, level: WittLevel::new(64) }).unwrap();
    let step = arena.push(Term::Literal { value: 1, level: WittLevel::new(64) }).unwrap();
    let r = arena.push(Term::Recurse {
        measure_index: measure,
        base_index: base,
        step_index: step,
