...
use hologram_ir::constant::{ConstantData, ConstantStore};
...
impl KvStore {
    ...
    pub fn dispatch_with_constants(
        ...
        Self::dispatch_with_shapes(op, inputs, constants, weights, &[], None)
    }
...
fn dispatch_lut_gemm_4(
    activation_bytes: &[u8],
    cid: hologram_ir::constant::ConstantId,
    constants: &ConstantStore,
    weights: &[u8],
) -> ExecResult<Vec<u8>> {
...
mod tests {
    ...
    fn dispatch_matmul_lut_missing_constant() {
        use hologram_ir::constant::ConstantId;
        let op = GraphOp::MatMulLut4(ConstantId::new(99));
        let act = [1.0f32; 4];
        let act_bytes: &[u8] = bytemuck::cast_slice(&act);
        let result =
            KvStore::dispatch_with_constants(&op, &[act_bytes], &ConstantStore::new(), &[]);
        assert!(result.is_err());
    }
}
