...
///
/// On non-Metal / non-WebGPU build targets the GPU-deferred variants are
/// `#[cfg]`-gated out: the enum reduces to `{ Skipped, Bytes }` and the
/// CPU dispatch hot path matches against exactly two arms. The per-call
/// variant cost claimed by Phase 2 demolition is already zero on the
/// default Linux build.
...
///
/// **⚠️ WebGPU is experimental — not validated in CI.** When the
/// `webgpu` Cargo feature is enabled, this function will return the
/// WebGPU backend on the first call where `WebGpuBackend::new()`
/// succeeds, but no conformance test in this workspace runs against
/// that path. See `backend/webgpu.rs` for the full caveat.
#[must_use]
