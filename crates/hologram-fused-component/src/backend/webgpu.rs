...
//!
//! ⚠️ **EXPERIMENTAL — NOT VALIDATED IN CI** ⚠️
//!
//! This module contains a complete WebGPU backend implementation
//! (~1100 LOC). It IS wired into `default_backend()` in `backend/mod.rs`
//! under `cfg(has_webgpu)` (set by `build.rs` when the `webgpu` Cargo
//! feature is active or when targeting `wasm32`), but **no test in this
//! workspace runs against this backend**. The Phase 10 conformance
//! refactor deferred WebGPU validation pending a working
//! Metal/WebGPU toolchain.
//!
//! Production builds that enable `--features webgpu` will route through
//! this code and inherit any latent bugs. Treat this backend as
//! experimental until conformance tests run against it (planned for a
//! future phase that lands a Metal/WebGPU CI runner).
//!
//! ──────────────────────────────────────────────────────────────────────
//!
