...
// `CustomOpRegistry` removed in v0.2.0 conformance-first refactor.
...
// `build_tape_from_plan_with_ops` removed in v0.2.0 conformance-first refactor
// along with `CustomOpRegistry` and `GraphOp::Custom`. The replacement
// extensibility model is Prism-modules-as-extensions, which dispatch at
// compile time. Perf: WIN — eliminates the function-pointer call.
...
fn seed_arena<'a>(
    ...
    compiled_dtypes: &std::collections::HashMap<
        hologram_ir::graph::node::NodeId,
        hologram_core::op::FloatDType,
    >,
    compiled_shapes: &std::collections::HashMap<hologram_ir::graph::node::NodeId, Vec<usize>>,
...
///
/// ```rust,ignore
/// use hologram_fused_component::{InferenceSession, GraphInputs};
///
/// let plan = hologram_archive::load_from_bytes(&archive)?;
/// let tape = hologram_fused_component::build_tape_from_plan(&plan)?;
///
...
mod tests {
    ...
    fn tape_lut_gemm_q4() {
        ...
        let g = hologram_ir::builder::GraphBuilder::new()
            .input("a")
            .node_from_graph_input(GraphOp::Input, 0)
    ...
    // `tape_custom_op_passthrough` test removed in v0.2.0 conformance-first
    // refactor. Custom ops are gone — see Prism modules as the replacement
    // extensibility model.
