...
///
/// Carries weight archive access, a lazily-populated weight cache for
/// LUT-GEMM ops, and an optional KV cache for autoregressive generation.
/// The compute backend is resolved once at the start of `execute_direct`
/// (via `default_backend()`); it is **not** carried on this struct, so
/// nothing in the inner dispatch loop touches a `BackendSelector` enum
/// or boxes a backend per kernel.
...
impl<'a> TapeContext<'a> {
    ...
    /// The compute backend is resolved at execute time, not stored here.
    #[must_use]
    pub fn new(
...
pub enum TapeKernel {
    ...
    // `Custom(CustomHandler)` removed in v0.2.0 conformance-first refactor:
    // custom ops violated the carrying criterion AND introduced runtime
    // function-pointer indirection. The replacement extensibility model is
    // Prism-modules-as-extensions, which dispatch at compile time. Perf: WIN.
    /// Group normalization.
    ...
    match kernel {
        ...
        TapeKernel::RingPrimUnary { op, level } => {
            ...
            if !input.is_empty() && out_buf.len() > base {
                ...
                flux.accumulate_at(curvature, level.to_witt_level());
            ...
            match effective_level {
                ...
                RingLevel::Q2 => {
                    ...
                    for (c_in, c_out) in input.chunks_exact(3).zip(dst.chunks_exact_mut(3)) {
                        ...
                        let r = op.apply::<hologram_ring::W32>(val) & 0x00FF_FFFF;
                        let b = r.to_le_bytes();
                        c_out[0] = b[0];
                        c_out[1] = b[1];
                        c_out[2] = b[2];
                ...
                RingLevel::Q3 => {
                    for (c_in, c_out) in input.chunks_exact(4).zip(dst.chunks_exact_mut(4)) {
                        ...
                        let r = op.apply::<hologram_ring::W32>(val);
                        c_out.copy_from_slice(&r.to_le_bytes());
                    }
                }
            }
...
impl EnumTape {
    ...
    pub fn compute_level_weight_ranges(
        &mut self,
        constants: &hologram_ir::constant::ConstantStore,
    ...
    pub fn finalize_consumer_counts_with_graph(
        ...
        for node in &sg.nodes {
            if matches!(node.op, hologram_ir::graph::GraphOp::Output) {
    ...
    ///
    /// **Backend resolution is hoisted** out of the inner loop: a single
    /// `default_backend()` call before the loop produces a `Box<dyn
    /// ComputeBackend>` that every instruction reuses through a `&dyn`
    /// reference. The Phase 10.6 cleanup removed the per-instruction
    /// `tape_ctx.backend.resolve()` call that was allocating a fresh
    /// boxed backend on every kernel dispatch.
    ///
    ...
    pub fn execute_direct(
        ...
        // Resolve the compute backend exactly once for the whole tape.
        // The boxed trait object lives for the duration of the executor;
        // every kernel dispatch uses `&*backend` (zero per-call cost).
        let backend = crate::backend::default_backend();
        let backend_ref: &dyn crate::backend::ComputeBackend = &*backend;
