...
pub(crate) fn reduce_sum(row: &[f32]) -> f32 {
    row.iter().sum()
}

pub(crate) fn reduce_mean(row: &[f32]) -> f32 {
    row.iter().sum::<f32>() / row.len() as f32
}

pub(crate) fn reduce_max(row: &[f32]) -> f32 {
    row.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
}

pub(crate) fn reduce_min(row: &[f32]) -> f32 {
    row.iter().cloned().fold(f32::INFINITY, f32::min)
}

pub(crate) fn reduce_prod(row: &[f32]) -> f32 {
    row.iter().product()
}
