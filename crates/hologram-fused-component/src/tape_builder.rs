...
/// # v0.2.0 reframing
///
/// The v0.1.4 `registry: Option<&CustomOpRegistry>` parameter is removed
/// because custom ops are gone. The replacement extensibility model is
/// Prism-modules-as-extensions, which dispatch at compile time. **Perf:
/// WIN** — eliminates the per-build registry lookup and the
/// per-instruction function-pointer call from custom op dispatch.
...
fn compute_weight_offset(
    ...
    match constants.get(cid) {
        Some(hologram_ir::constant::ConstantData::Deferred { source_id, .. }) => *source_id as u32,
        _ => 0,
    }
}
