...
fn ring_activation_relu_q0_correct() {
    ...
    for x in 0u8..=255 {
        let result = ActivationOp::Relu.apply::<hologram_ring::W8>(x);
        if x <= 127 {
...
fn ring_activation_sigmoid_q3_monotonic() {
    ...
    let mut prev = ActivationOp::Sigmoid.apply::<hologram_ring::W32>(0u32);
    for x in 1..=1000u32 {
        let cur = ActivationOp::Sigmoid.apply::<hologram_ring::W32>(x * (u32::MAX / 1000));
...
fn ring_activation_gelu_q0_non_identity() {
    ...
    for x in 0u8..=255 {
        if ActivationOp::Gelu.apply::<hologram_ring::W8>(x) != x {
            non_identity += 1;
        }
    }
    assert!(
...
fn ring_activation_square_q3_known_answer() {
    // Square(7) = 49
    assert_eq!(ActivationOp::Square.apply::<hologram_ring::W32>(7u32), 49);
    // Square(0) = 0
    assert_eq!(ActivationOp::Square.apply::<hologram_ring::W32>(0u32), 0);
}
...
fn tape_kernel_ring_activation_exists() {
    use hologram_fused_component::tape::TapeKernel;
