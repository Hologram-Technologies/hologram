...
use hologram_backend::{
    Backend, KernelCall, BufferRef, BinaryCall, MatMulCall, Workspace,
};
...
fn wgpu_add_f32_smoke() {
    let mut backend: WgpuBackend<Ws> = match WgpuBackend::new() {
        Ok(b) => b,
        Err(_) => { eprintln!("wgpu unavailable — skipping"); return; }
    };
    let a = f32_to_le(&[1.0, 2.0, 3.0, 4.0]);
    let b = f32_to_le(&[10.0, 20.0, 30.0, 40.0]);
    let mut ws = Ws { slots: vec![a, b, vec![0u8; 16]] };
    let call = KernelCall::Add(BinaryCall {
        a: buf(0), b: buf(1), output: buf(2),
...
fn wgpu_falls_back_to_cpu_for_byte_kernels() {
    ...
    let mut ws = Ws { slots: vec![vec![1u8, 2, 3], vec![10u8, 20, 30], vec![0u8; 3]] };
    let call = KernelCall::Add(BinaryCall {
        a: buf(0), b: buf(1), output: buf(2),
        element_count: 3, witt_bits: 8, dtype: 1,
    });
    backend.dispatch(&call, &mut ws).unwrap();
