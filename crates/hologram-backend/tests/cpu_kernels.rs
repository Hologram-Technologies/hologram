...
use hologram_backend::{
    ...
    UnaryCall, BinaryCall, MatMulCall, LayoutCall, ReduceCall, SoftmaxCall,
    Workspace,
};
...
impl TestWorkspace {
    fn new(slots: Vec<Vec<u8>>) -> Self { Self { slots } }
    fn slot(&self, i: u32) -> &Vec<u8> { &self.slots[i as usize] }
    fn slot_mut(&mut self, i: u32) -> &mut Vec<u8> { &mut self.slots[i as usize] }
}

impl Workspace for TestWorkspace {
    fn read(&self, b: BufferRef) -> &[u8] { &self.slot(b.slot)[..] }
...
fn add_byte_kernel() {
    ...
    let call = KernelCall::Add(BinaryCall {
        a: buf(0, 4), b: buf(1, 4), output: buf(2, 4),
        element_count: 4, witt_bits: 8, dtype: 1,
    });
...
fn reshape_copies_input() {
    ...
    let call = KernelCall::Reshape(LayoutCall {
        input: buf(0, 6), output: buf(1, 6),
        element_count: 6, dtype: 0,
    });
    backend.dispatch(&call, &mut ws).unwrap();
...
fn reduce_sum_byte() {
    ...
    let call = KernelCall::ReduceSum(ReduceCall {
        input: buf(0, 5), output: buf(1, 1),
        element_count: 5, axis_count: 1, keepdims: false, dtype: 0,
    });
    backend.dispatch(&call, &mut ws).unwrap();
