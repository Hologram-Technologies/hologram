...
use hologram_backend::{
    ...
    UnaryCall, BinaryCall, MatMulCall,
    Workspace,
};
use hologram_backend::cpu::dtype::DTYPE_F32;

struct TestWorkspace { slots: Vec<Vec<u8>> }
...
fn buf(slot: u32, length: u32) -> BufferRef {
    BufferRef { slot, offset: 0, length }
}
...
fn add_f32_native() {
    ...
    let mut ws = TestWorkspace { slots: vec![a, b, vec![0u8; 16]] };
    let mut backend: CpuBackend<TestWorkspace> = CpuBackend::new();
    let call = KernelCall::Add(BinaryCall {
        a: buf(0, 16), b: buf(1, 16), output: buf(2, 16),
        element_count: 4, witt_bits: 32, dtype: DTYPE_F32,
    });
...
fn relu_f32_native() {
    ...
    let mut ws = TestWorkspace { slots: vec![a, vec![0u8; 20]] };
    let mut backend: CpuBackend<TestWorkspace> = CpuBackend::new();
    let call = KernelCall::Relu(UnaryCall {
        input: buf(0, 20), output: buf(1, 20),
        element_count: 5, witt_bits: 32, dtype: DTYPE_F32,
    });
    backend.dispatch(&call, &mut ws).unwrap();
...
fn sigmoid_f32_native() {
    let a = f32_to_le(&[0.0]);
    let mut ws = TestWorkspace { slots: vec![a, vec![0u8; 4]] };
    let mut backend: CpuBackend<TestWorkspace> = CpuBackend::new();
    let call = KernelCall::Sigmoid(UnaryCall {
        input: buf(0, 4), output: buf(1, 4),
        element_count: 1, witt_bits: 32, dtype: DTYPE_F32,
    });
    backend.dispatch(&call, &mut ws).unwrap();
    let out = le_to_f32(&ws.slots[1]);
    // sigmoid(0) == 0.5
...
fn tanh_f32_native() {
    ...
    let mut ws = TestWorkspace { slots: vec![a, vec![0u8; 8]] };
    let mut backend: CpuBackend<TestWorkspace> = CpuBackend::new();
    let call = KernelCall::Tanh(UnaryCall {
        input: buf(0, 8), output: buf(1, 8),
        element_count: 2, witt_bits: 32, dtype: DTYPE_F32,
    });
    backend.dispatch(&call, &mut ws).unwrap();
    let out = le_to_f32(&ws.slots[1]);
