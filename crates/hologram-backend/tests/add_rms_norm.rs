...
use hologram_backend::{
    Backend, KernelCall, BufferRef, NormCall, Workspace, CpuBackend,
};
...
impl Workspace for Ws {
    ...
    fn split_borrow<'a>(
        ...
        if reads.iter().any(|r| r.slot != u32::MAX && r.slot as usize == w) { return None; }
        let (lo, hi) = self.slots.split_at_mut(w);
        let (wbuf, hi_rest) = hi.split_first_mut()?;
        let rs = reads.iter().map(|r| -> &[u8] {
            if r.slot == u32::MAX { return &[]; }
...
fn buf(slot: u32) -> BufferRef { BufferRef { slot, offset: 0, length: 0 } }
...
fn add_rms_norm_sums_residual_before_normalizing() {
    ...
    let mut ws = Ws { slots: vec![
        x, gamma, vec![],   // beta unused for rms
        resid, vec![0u8; 16],
    ]};
    let mut backend: CpuBackend<Ws> = CpuBackend::new();
    let call = KernelCall::AddRmsNorm(NormCall {
        x: buf(0), gamma: buf(1), beta: NormCall::NO_RESIDUAL,
        residual: buf(3), output: buf(4),
        batch: 1, feature: 4,
        epsilon_bits: 0, dtype: DTYPE_F32,
    });
...
fn add_rms_norm_no_residual_falls_back_to_rms() {
    ...
    let gamma = f32_vec(&[1.0, 1.0, 1.0, 1.0]);
    let mut ws = Ws { slots: vec![
        x, gamma, vec![], vec![0u8; 16],
    ]};
    let mut backend: CpuBackend<Ws> = CpuBackend::new();
    let call = KernelCall::AddRmsNorm(NormCall {
        x: buf(0), gamma: buf(1), beta: NormCall::NO_RESIDUAL,
        residual: NormCall::NO_RESIDUAL, output: buf(3),
        batch: 1, feature: 4,
        epsilon_bits: 0, dtype: DTYPE_F32,
    });
    backend.dispatch(&call, &mut ws).unwrap();
