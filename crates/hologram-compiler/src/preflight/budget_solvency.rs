//! CS_6: Budget Solvency Check — `cascade:BudgetSolvencyCheck` at preflightOrder 0.
//!
//! Rejects a CompileUnit whose declared `thermodynamicBudget` is strictly less
//! than the Landauer minimum: `bitsWidth(Q_k) × ln(2)` in k_B T units.
//!
...
///
/// Returns `true` if the declared budget meets or exceeds the Landauer minimum
/// for the unit's quantum level. Returns `false` if the unit should be rejected.
///
/// Implements `cascade:BudgetSolvencyCheck` at `preflightOrder = 0`
/// (per `uor_foundation::kernel::cascade::budget_solvency_check::PREFLIGHT_ORDER`).
#[inline]
pub fn check_budget_solvency(unit: &HoloCompileUnit) -> bool {
    unit.thermodynamic_budget >= minimum_budget(unit.quantum_level)
}
