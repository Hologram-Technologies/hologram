...
use hologram_graph::{Graph, ConvAttrs, GraphOp, InputSource, Node, NodeId, OpKind};
...
fn conv_attrs_thread_through_compile() {
    ...
    let mut g = Graph::new();
    let shape_x = g.shape_registry_mut().intern(ShapeDescriptor::rank4(1, 1, 4, 4));
    let shape_w = g.shape_registry_mut().intern(ShapeDescriptor::rank4(1, 1, 3, 3));
    let shape_y = g.shape_registry_mut().intern(ShapeDescriptor::rank4(1, 1, 2, 2));
    let dtype = DTypeId(8); // F32
    let x = g.add_node(Node {
        op: GraphOp::Input, inputs: SmallVec::new(),
        output_dtype: dtype, output_shape: shape_x,
    });
    g.add_input(x);
    let w = g.add_node(Node {
        op: GraphOp::Input, inputs: SmallVec::new(),
        output_dtype: dtype, output_shape: shape_w,
    });
    g.add_input(w);
    let conv = g.add_node(Node {
        op: GraphOp::Op(OpKind::Conv2d),
        inputs: SmallVec::from_iter([InputSource::Node(x), InputSource::Node(w)]),
        output_dtype: dtype, output_shape: shape_y,
    });
    g.set_conv_attrs(NodeId(conv.0), ConvAttrs { stride_h: 2, stride_w: 2, pad_h: 1, pad_w: 1 });
    ...
    let archive = Compiler::new(g, BackendKind::Cpu, WittLevel::W32)
        .compile().expect("conv graph compiles");
    let plan = hologram_archive::HoloLoader::from_bytes(&archive.archive)
