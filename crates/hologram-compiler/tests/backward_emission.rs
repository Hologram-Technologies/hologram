...
use hologram_graph::{Graph, GraphOp, InputSource, OpKind, append_backward};
...
fn append_backward_grows_graph_with_grad_nodes() {
    ...
    let added_unary_grad = graph.nodes().iter()
        .any(|n| matches!(n.op, GraphOp::Op(OpKind::UnaryGrad)));
    assert!(added_unary_grad, "expected a UnaryGrad node in backward subgraph");
}
...
fn primary_grad_mapping_is_consistent() {
    ...
    assert_eq!(OpKind::LayerNorm.primary_grad(),  Some(OpKind::LayerNormGrad));
...
fn compile_with_backward_returns_input_gradients() {
    ...
    let (output, input_grads) = compile_with_backward(
        graph,
        y,
        BackendKind::Cpu,
        WittLevel::W32,
