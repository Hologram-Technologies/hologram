...
impl core::hash::Hash for CertKey {
    ...
    fn hash<H: core::hash::Hasher>(&self, state: &mut H) {
        ...
        state.write_u32(self.level.witt_length());
    }
}
...
impl CertificateStore {
    ...
    fn hash_key(&self, key: &CertKey) -> usize {
        ...
        h ^= key.level.witt_length() as u64;
...
impl CertificateStore {
    ...
    pub fn save(&self, path: &std::path::Path) -> std::io::Result<()> {
        ...
        for (key, cert) in self.slots.iter().flatten() {
            ...
            buf.push(key.level.witt_length() as u8);
...
mod tests {
    ...
    fn insert_and_lookup() {
        ...
        let cert = make_cert(42, QuantumLevel::W8);
        store.insert(cert.clone());

        let result = store.get(&cert.unit_address, QuantumLevel::W8);
    ...
    fn lookup_miss() {
        ...
        let mut addr = [0u8; 32];
        addr[0] = 99;
        assert!(store.get(&addr, QuantumLevel::W8).is_none());
    }

    #[test]
    fn different_levels_different_entries() {
        let mut store = CertificateStore::new(64);
        let cert_q0 = make_cert(1, QuantumLevel::W8);
        let cert_q1 = Certificate {
            quantum_level: QuantumLevel::W16,
            budget_consumed: 10.0,
            ..make_cert(1, QuantumLevel::W16)
        };
        ...
        let r0 = store.get(&cert_q0.unit_address, QuantumLevel::W8).unwrap();
        assert_eq!(r0.budget_consumed, 5.0);

        let r1 = store.get(&cert_q1.unit_address, QuantumLevel::W16).unwrap();
    ...
    fn update_existing() {
        let mut store = CertificateStore::new(64);
        let cert1 = make_cert(1, QuantumLevel::W8);
        store.insert(cert1);

        let cert2 = Certificate {
            budget_consumed: 99.0,
            ..make_cert(1, QuantumLevel::W8)
        };
        store.insert(cert2);
        ...
        let r = store
            .get(
                &make_cert(1, QuantumLevel::W8).unit_address,
                QuantumLevel::W8,
            )
            .unwrap();
    ...
    fn many_entries() {
        ...
        for i in 0..200u8 {
            store.insert(make_cert(i, QuantumLevel::W8));
        }
        ...
        for i in 0..200u8 {
            ...
            addr[0] = i;
            assert!(
                store.get(&addr, QuantumLevel::W8).is_some(),
    ...
    fn save_load_round_trip() {
        let mut store = CertificateStore::new(64);
        store.insert(make_cert(1, QuantumLevel::W8));
        store.insert(make_cert(2, QuantumLevel::W16));
        store.insert(make_cert(3, QuantumLevel::W8));
        ...
        addr1[0] = 1;
        let cert = loaded.get(&addr1, QuantumLevel::W8).unwrap();
        assert!(cert.converged);
        assert_eq!(cert.budget_consumed, 5.0);

        let mut addr2 = [0u8; 32];
        addr2[0] = 2;
        assert!(loaded.get(&addr2, QuantumLevel::W16).is_some());
    ...
    fn certificate_lookup_performance() {
        ...
        for _ in 0..1_000_000 {
            let _ = store.get(&addr, QuantumLevel::W8);
        }
