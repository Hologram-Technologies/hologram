...
fn decode_step_matches_independent_reference_bitwise() {
    ...
    for hi in 0..h {
        ...
        for (j, slot) in scores.iter_mut().enumerate() {
            ...
            let dot = hologram_compute::cpu::simd::simd_f32_dot(qrow, krow);
        ...
        hologram_compute::cpu::simd::simd_f32_exp_inplace(&mut scores);
        let mut sum = 0f32;
        for &v in &scores {
            sum += v;
        }
        ...
        for (j, &sc) in scores.iter().enumerate() {
            ...
            hologram_compute::cpu::simd::simd_f32_axpy(orow, p, vrow);
        }
    }
