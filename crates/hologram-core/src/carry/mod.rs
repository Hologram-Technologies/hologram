...
impl CurvatureFlux {
    ...
    #[inline]
    pub fn accumulate_at(&mut self, curvature: u8, level: uor_foundation::WittLevel) {
        self.carry += curvature as u64;
        if curvature > 0 {
            let w = level.byte_width();
            if w > self.max_carry_width {
...
mod tests {
    ...
    fn lift_lower_round_trip_q0_q1() {
        ...
        for x in 0u8..=255 {
            let lifted = lift(x as u64, QuantumLevel::W8, QuantumLevel::W16);
            let lowered = lower(lifted, QuantumLevel::W16, QuantumLevel::W8)
    ...
    fn lower_fails_for_high_byte() {
        assert!(lower(0x0100, QuantumLevel::W16, QuantumLevel::W8).is_err());
        assert!(lower(0xFF00, QuantumLevel::W16, QuantumLevel::W8).is_err());
        assert_eq!(
            lower(0x0100, QuantumLevel::W16, QuantumLevel::W8).unwrap_err(),
            0x0100
        );
    }
    ...
    fn lift_lower_round_trip_q1_q2() {
        for x in [0u16, 1, 127, 255, 256, 0x7FFF, 0x8000, 0xFFFE, 0xFFFF] {
            let lifted = lift(x as u64, QuantumLevel::W16, QuantumLevel::W24);
            let lowered = lower(lifted, QuantumLevel::W24, QuantumLevel::W16)
                .expect("round-trip must succeed");
    ...
    fn lower_q2_to_q1_fails_for_high_bits() {
        assert!(lower(0x00_010000, QuantumLevel::W24, QuantumLevel::W16).is_err());
        assert!(lower(0x00FF_0000, QuantumLevel::W24, QuantumLevel::W16).is_err());
    }
    ...
    fn lift_q0_to_q2_is_exact() {
        for x in [0u8, 1, 127, 255] {
            let q2 = lift(x as u64, QuantumLevel::W8, QuantumLevel::W24);
