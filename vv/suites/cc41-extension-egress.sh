...
# (`TCPSocket`). The holospaces egress extension (`spaces/holospaces-browser/
...
# FAILED (no silent drop). Witness: spaces/holospaces-browser/extension/egress-test.mjs.

set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

if ! command -v node >/dev/null 2>&1; then
    echo "cc41-extension-egress: FAIL — node not available in this environment" >&2
    exit 127
fi

node "$ROOT/spaces/holospaces-browser/extension/egress-test.mjs" || exit 1
...
command -v zip >/dev/null 2>&1 \
    || { echo "cc41-extension-egress: FAIL — zip is required for the package witness" >&2; exit 127; }
command -v unzip >/dev/null 2>&1 \
    || { echo "cc41-extension-egress: FAIL — unzip is required for the package witness" >&2; exit 127; }
