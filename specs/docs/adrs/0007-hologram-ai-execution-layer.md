...
owns a `BufferArena` per session for managing persistent state (KV-cache). This session
arena is separate from the executor's internal arena — `KvExecutor::execute()` creates its
own `BufferArena` internally per call. The session injects cached data via `GraphInputs`
and extracts results from `GraphOutputs`. See `execution.md` §BufferArena Ownership for
