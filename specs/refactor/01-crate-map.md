...
| Current | Target | Notes |
|---------|--------|-------|
| holospaces/crates/holospaces — the generic container-lifecycle `Session` (boot/suspend/resume/terminate over the `ContainerRuntime` trait) | **hoisted to crates/hologram-runtime** (`lifecycle` module) | D7 **as realized 2026-07-15**: only the lifecycle *primitive* is space-agnostic. It holds a container-manifest κ + capability-set κ; holospaces keeps a thin `Session` wrapper (adds `holospace()`/`reconfigure()`) so Peer/Manager/CC tests are unchanged |
| holospaces/crates/holospaces — Peer, Manager, Operator/Roster (identity.rs), Configuration (config.rs) | **stay in spaces/holospaces** | correction to the original D7: Peer (provisions `Holospace`s, walks the holospaces realization registry) and Manager (operator console — roster sync, control-plane `Configuration`/reconfigure, ADR-018) are holospaces' *platform* layer, generic over `R` only mechanically — not generic infrastructure |
