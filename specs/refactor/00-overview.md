...
3. **Contracts are hologram's.** The space contract is defined once in `hologram-space`;
   every space implements the identical surface. Platform differences live *behind* the
   traits, never in them. Conformance = passing `hologram-tck`. (Decision D2.)
